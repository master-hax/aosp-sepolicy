...
def TestIsolatedAttributeConsistency(test_policy):
  ...
  def checkPermissions(permissions):
    ...
    for perm in permissions:
      target_context, target_class, p = perm.split(":")
      # if the target context is an attribute, check the members too
      target_contexts = [target_context] + \
          ( test_policy.pol.QueryTypeAttribute(Type=target_context, IsAttr=True)
            if target_context in test_policy.pol.GetAllTypes(True)
            else [] )
      for ctx in target_contexts:
        if ctx not in permissionAllowList or target_class not in permissionAllowList[ctx] \
