...
def do_main():
    ...
    if not options.current or not options.prebuilt:
        sys.exit("Must specify both corrent and prebuilt\n" + parser.usage)
    ...
    removed_attributes = set(filter(lambda x: "base_typeattr_" not in x, prebuilt_policy.typeattributes - current_policy.typeattributes))

    if removed_types:
        results += "The following public types are removed:\n" + ", ".join(removed_types) + "\n"

    if removed_attributes:
        results += "The following public attributes are removed:\n" + ", ".join(removed_attributes) + "\n"
