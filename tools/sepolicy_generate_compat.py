...
def build_base_files(target_version):
    ...
    Returns:
      (string, string, string), paths to base policy, old policy, and pub policy cil
    """
    ...
    old_policy_path = os.path.join(dist_dir, '%s_plat_sepolicy' % target_version)
...
def main():
    ...
    try:
        ...
        target_compat_path = os.path.join(sepolicy_path, "private", "compat", args.target_version)

        # Step 1. Download system/etc/selinux/mapping/{ver}.cil, and remove types/typeattributes
        mapping_file = download_mapping_file(args.branch, args.build, args.target_version)
        ...
        base_policy_path, old_policy_path, pub_policy_cil_path = build_base_files(args.target_version)
        ...
        latest_compat_path = os.path.join(sepolicy_path, "private", "compat", args.latest_version)
        latest_combined_mapping_cil_path = os.path.join(temp_dir, "%s_mapping.combined.cil" % args.latest_version)
        with open(latest_combined_mapping_cil_path, 'w') as fw:
            with open(os.path.join(latest_compat_path, args.latest_version + ".cil"), 'r') as fi:
                fw.write(fi.read())
            with open(os.path.join(latest_compat_path, args.latest_version + ".ignore.cil"), 'r') as fi:
                fw.write(fi.read())
        latest_mapping_cil = mini_parser.MiniCilParser(latest_combined_mapping_cil_path)

        # Step 3. Find new types and removed types
        new_types = pub_types & (all_types - old_all_types)
        removed_types = latest_mapping.pubtypes & (old_all_types - all_types)
