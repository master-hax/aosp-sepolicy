...
static void usage(char *name) {
    fprintf(stderr, "usage1:  %s [-l|-p|-s|-v] [-e] sepolicy context_file\n\n"
        ...
        "Validates a file against test_data. test_data is a text file where each line has the format:\n"
        "  path expected_type\n\n\n",
        name, name, name);
    exit(1);
}
...
static void do_test_data_and_die_on_error(struct selinux_opt opts[], unsigned int backend, char *paths[])
{
