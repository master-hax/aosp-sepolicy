...
int main(int argc, char *argv[])
{
    ...
    if (binary_out == NULL) {
        perror("Failure opening binary file for writing");
        rc = SEPOL_ERR;
        goto exit;
    }
...
exit:
    if (binary_base != NULL && fclose(binary_base) == EOF && rc == SEPOL_OK) {
        perror("Failure closing input binary file");
        rc = SEPOL_ERR;
    }
    if (binary_out != NULL && fclose(binary_out) == EOF && rc == SEPOL_OK) {
        perror("Failure closing output binary file");
        rc = SEPOL_ERR;
    }
    exit(rc);
}
