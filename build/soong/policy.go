...
func (c *policyConf) transformPolicyToConf(ctx android.ModuleContext) android.OutputPath {
	conf := android.PathForModuleOut(ctx, ctx.Config().DeviceName(), c.stem()).OutputPath
...
func (c *policyCil) compileConfToCil(ctx android.ModuleContext, conf android.Path) android.OutputPath {
	cil := android.PathForModuleOut(ctx, ctx.Config().DeviceName(), c.stem()).OutputPath
	rule := android.NewRuleBuilder(pctx, ctx)
...
func (c *policyBinary) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	bin := android.PathForModuleOut(ctx, ctx.Config().DeviceName(), c.stem()+"_policy")
	rule := android.NewRuleBuilder(pctx, ctx)
	secilcCmd := rule.Command().BuiltTool("secilc").
		Flag("-m").                 // Multiple decls
		FlagWithArg("-M ", "true"). // Enable MLS
	...
	if !ctx.Config().Debuggable() {
		permissiveDomains := android.PathForModuleOut(ctx, ctx.Config().DeviceName(), c.stem()+"_permissive")
	...
	out := android.PathForModuleOut(ctx, ctx.Config().DeviceName(), c.stem())
