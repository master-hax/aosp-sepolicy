...
func (n *neverallowTestModule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	ctx.VisitDirectDeps(func(child android.Module) {
		...
		if !ok {
			ctx.ModuleErrorf("Module %q isn't an OutputFileProducer", ctx.OtherModuleName(child))
			return
		}

		outputs, err := o.OutputFiles("")
		if err != nil {
			ctx.ModuleErrorf("Module %q error while producing output: %v", ctx.OtherModuleName(child), err)
			return
		}
	...
	if len(checkpolicyConfPaths) != 1 {
		ctx.ModuleErrorf("Module %q should produce exactly one output", n.checkpolicyConfModuleName())
		return
	}

	if len(sepolicyAnalyzeConfPaths) != 1 {
		ctx.ModuleErrorf("Module %q should produce exactly one output", n.sepolicyAnalyzeConfModuleName())
		return
	}
