...
func (fg *fileGroup) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	productVars := ctx.Config().ProductVariables
	srcDirsMap := map[string][]string{
		"BOARD_PLAT_PUBLIC_SEPOLICY_DIR": []string{
			productVars.BoardPlatPrivateSepolicyDir},
		"BOARD_PLAT_PRIVATE_SEPOLICY_DIR": []string{
			productVars.BoardPlatPrivateSepolicyDir},
		"BOARD_SEPOLICY_DIRS":     productVars.BoardVendorSepolicyDirs,
		"BOARD_ODM_SEPOLICY_DIRS": productVars.BoardOdmSepolicyDirs,
	}
	...
	for _, d := range fg.properties.SrcDirs {
		...
		if !ok {
			ctx.ModuleErrorf("se_filegroup is only for files in sepolicy directories.")
		}
