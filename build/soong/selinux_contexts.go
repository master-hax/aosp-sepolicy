...
func (m *selinuxContextsModule) buildPropertyContexts(ctx android.ModuleContext, inputs android.Paths) android.Path {
	...
	// vendor/odm properties are enforced for devices launching with Android Q (29) or later.
	// So, if vendor/odm, make sure that only vendor/odm properties exist
	if (ctx.SocSpecific() || ctx.DeviceSpecific()) && shippingApiLevel >= 29 {
		rule := android.NewRuleBuilder(pctx, ctx)
		...
		// persist.camera is also allowed for devices launching with R (30) or eariler
		if shippingApiLevel <= 30 {
		...
		invalidPropertyMsg := ctx.ModuleName() + ` contains properties which are not vendor or odm namespaced.\n` +
			`Every property in ` + ctx.ModuleName() + ` should start with vendor or odm prefixes.\n` +
			`Please fix such offending properties.\n` +
			`Lists are:\n`
		...
		rule.Command().
			...
			Input(builtCtxFile).Text(";	exit 38) )")

		// Starting from Android R (30), vendor contexts are also enforced by VtsTrebleSysPropTest.
		if shippingApiLevel >= 30 {
			...
			invalidContextMsg := ctx.ModuleName() + ` contains property contexts which are not vendor or odm namespaced.\n` +
				`Every context used in ` + ctx.ModuleName() + ` should start with vendor_ or odm_.\n` +
				`Please fix such offending property contexts.\n` +
				`Lists are:\n`
