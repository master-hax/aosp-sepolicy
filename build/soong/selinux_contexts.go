...
func (m *selinuxContextsModule) productVariableSrcs(config android.Config) []string {
	var srcs []string
...
func (m *selinuxContextsModule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	srcs := append(m.properties.Srcs, m.productVariableSrcs(ctx.Config())...)
...
func (m *selinuxContextsModule) DepsMutator(ctx android.BottomUpMutatorContext) {
	if !m.inRecovery() || m.onlyInRecovery() {
		android.ExtractSourcesDeps(ctx, m.productVariableSrcs(ctx.Config()))
	}
}
