...
$(error RPODUCT_FULL_TREBLE needs to be true when using BOARD_ODM_SEPOLICY_DIRS)
endif
endif
...
#################################
# Both vendor sepolicy and odm sepolicy need $(PLAT_PUBLIC_POLICY) and
# $(REQD_MASK_POLICY) to pass an intermediate check via "checkpolicy".
# In order to prevent Re-declaration, plat_pub_versioned.cil is
# introduced to include the two files. It will be combined with vendor and
# odm sepolicy temporarily during "checkpolicy" check.
# plat_pub_versioned.cil will be a separate output file to /vendor,
# to be merged with vendor_sepolicy.cil and/or odm_sepolicy.cil when the
# device boots.
#################################
include $(CLEAR_VARS)
...
built_plat_pub_cil := $(LOCAL_BUILT_MODULE)
...
$(LOCAL_BUILT_MODULE) : PRIVATE_DEP_CIL_FILES := $(built_plat_cil) $(built_plat_pub_cil) $(built_mapping_cil)
$(LOCAL_BUILT_MODULE) : PRIVATE_FILTER_CIL := $(built_plat_pub_cil)
$(LOCAL_BUILT_MODULE) : $(plat_pub_policy.cil) $(vendor_policy_raw) \
  $(HOST_OUT_EXECUTABLES)/version_policy $(HOST_OUT_EXECUTABLES)/secilc \
  $(built_plat_cil) $(built_plat_pub_cil) $(built_mapping_cil)
	@mkdir -p $(dir $@)
...
$(PLAT_PUBLIC_POLICY) $(REQD_MASK_POLICY) $(BOARD_ODM_SEPOLICY_DIRS))
	$(transform-policy-to-conf)
	$(hide) sed '/dontaudit/d' $@ > $@.dontaudit
