...
# nonplat_filter.cil is introduced to include the three
# files. It will be combined with vendor and odm sepolicy temporarily
# during "checkpolicy" check. nonplat_filter.cil will be a separate
# output file, to be merged with vendor_sepolicy.cil and/or
# odm_sepolicy.cil when device boots.
#################################
include $(CLEAR_VARS)

LOCAL_MODULE := nonplat_filter.cil
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_TAGS := optional
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_PATH := $(TARGET_OUT_VENDOR)/etc/selinux/filter

include $(BUILD_SYSTEM)/base_rules.mk

nonplat_filter.conf := $(intermediates)/nonplat_filter.conf
$(nonplat_filter.conf): PRIVATE_MLS_SENS := $(MLS_SENS)
$(nonplat_filter.conf): PRIVATE_MLS_CATS := $(MLS_CATS)
$(nonplat_filter.conf): PRIVATE_TGT_ARCH := $(my_target_arch)
$(nonplat_filter.conf): PRIVATE_TGT_WITH_ASAN := $(with_asan)
$(nonplat_filter.conf): PRIVATE_ADDITIONAL_M4DEFS := $(LOCAL_ADDITIONAL_M4DEFS)
$(nonplat_filter.conf): PRIVATE_FULL_TREBLE := $(PRODUCT_FULL_TREBLE)
$(nonplat_filter.conf): $(call build_policy, $(sepolicy_build_files), \
$(PLAT_PUBLIC_POLICY) $(REQD_MASK_POLICY) $(PLAT_VENDOR_POLICY))
	$(transform-policy-to-conf)
	$(hide) sed '/dontaudit/d' $@ > $@.dontaudit

nonplat_filter_raw := $(intermediates)/nonplat_filter_raw.cil
$(nonplat_filter_raw): PRIVATE_POL_CONF := $(nonplat_filter.conf)
$(nonplat_filter_raw): PRIVATE_REQD_MASK := $(reqd_policy_mask.cil)
$(nonplat_filter_raw): $(HOST_OUT_EXECUTABLES)/checkpolicy $(nonplat_filter.conf) \
...
$(LOCAL_BUILT_MODULE) : PRIVATE_TGT_POL := $(nonplat_filter_raw)
$(LOCAL_BUILT_MODULE) : PRIVATE_DEP_CIL_FILES := $(built_plat_cil) $(built_mapping_cil)
$(LOCAL_BUILT_MODULE) : $(plat_pub_policy.cil) $(nonplat_filter_raw) \
...
built_nonplat_filter_cil := $(LOCAL_BUILT_MODULE)
nonplat_filter.conf :=
nonplat_filter_raw :=
...
$(HOST_OUT_EXECUTABLES)/version_policy $(HOST_OUT_EXECUTABLES)/secilc \
$(built_plat_cil) $(built_mapping_cil) $(built_nonplat_filter_cil)
	@mkdir -p $(dir $@)
	$(HOST_OUT_EXECUTABLES)/version_policy -b $< -t $(PRIVATE_TGT_POL) -n $(PRIVATE_VERS) -o $@.tmp
	$(hide) grep -Fxv -f $(built_nonplat_filter_cil) $@.tmp > $@
...
$(built_plat_cil) $(built_mapping_cil) $(built_nonplat_filter_cil) $(built_vendor_cil) $(built_odm_cil)
