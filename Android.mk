...
plat_policy.conf :=
built_plat_policy := $(LOCAL_BUILT_MODULE)
...
$(LOCAL_BUILT_MODULE): $(mapping.cil) $(ACP)
	$(copy-file-to-new-target)

current_mapping.cil :=
mapping.cil :=
built_mapping_policy := $(LOCAL_BUILT_MODULE)
...
$(nonplat_policy.cil): PRIVATE_POL_CONF := $(nonplat_policy.conf)
$(nonplat_policy.cil): PRIVATE_REQD_MASK := $(reqd_policy_mask.cil)
$(nonplat_policy.cil): $(HOST_OUT_EXECUTABLES)/checkpolicy $(nonplat_policy.conf) \
$(reqd_policy_mask.cil)
	@mkdir -p $(dir $@)
	$(hide) $< -C -M -c $(POLICYVERS) -o $@.tmp $(PRIVATE_POL_CONF)
	$(hide) grep -Fxv -f $(PRIVATE_REQD_MASK) $@.tmp | grep -v neverallow > $@

$(LOCAL_BUILT_MODULE) : PRIVATE_VERS := $(BOARD_SEPOLICY_VERS)
$(LOCAL_BUILT_MODULE) : PRIVATE_TGT_POL := $(nonplat_policy.cil)
$(LOCAL_BUILT_MODULE) : $(plat_pub_policy.cil) $(nonplat_policy.cil) \
...
nonplat_policy.conf :=
nonplat_policy.cil :=
built_nonplat_policy := $(LOCAL_BUILT_MODULE)
...
all_cil_files := \
    $(built_plat_policy) \
    $(built_mapping_policy) \
    $(built_nonplat_policy) \

$(LOCAL_BUILT_MODULE): PRIVATE_CIL_FILES := $(all_cil_files)
$(LOCAL_BUILT_MODULE): $(HOST_OUT_EXECUTABLES)/secilc $(HOST_OUT_EXECUTABLES)/sepolicy-analyze $(all_cil_files)
	@mkdir -p $(dir $@)
	$(hide) $< -M true -c $(POLICYVERS) $(PRIVATE_CIL_FILES) -o $@.tmp #> /dev/null
