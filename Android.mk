...
ifneq (,$(PRODUCT_PUBLIC_POLICY))
HAS_PRODUCT_SEPOLICY := true
endif
ifneq (,$(PRODUCT_PRIVATE_POLICY))
HAS_PRODUCT_SEPOLICY := true
endif
...
$(HOST_OUT_EXECUTABLES)/secilc $(built_plat_cil)
	@mkdir -p $(dir $@)
	$(hide) $(CHECKPOLICY_ASAN_OPTIONS) $(HOST_OUT_EXECUTABLES)/checkpolicy -M -C -c \
	$(POLICYVERS) -o $@.tmp $<
	$(hide) grep -Fxv -f $(PRIVATE_PLAT_CIL) $@.tmp > $@
	# Line markers (denoted by ;;) are malformed after above cmd. They are only
	# used for debugging, so we remove them.
	$(hide) sed -i '/;;/d' $@
	# product_sepolicy.cil can only be turned into binary policy in combination
	# with plat_sepolicy.cil, so we don't use secilc to test
	# product_sepolicy.cil here.
