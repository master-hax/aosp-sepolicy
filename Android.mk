...
# BOARD_SEPOLICY_DIRS is used for vendor/odm sepolicy customization before.
# It has been replaced by BOARD_VENDOR_SEPOLICY_DIRS (mandatory) and
# BOARD_ODM_SEPOLICY_DIRS (optional).
ifdef BOARD_SEPOLICY_DIRS
ifdef BOARD_VENDOR_SEPOLICY_DIRS
$(error BOARD_SEPOLICY_DIRS is deprecated, use BOARD_VENDOR_SEPOLICY_DIRS instead)
endif
...
build_odm_policy = $(call build_policy, $(1), $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS))
...
#################################
# Both vendor sepolicy and odm sepolicy need $(PLAT_PUBLIC_POLICY),
# $(REQD_MASK_POLICY) and $(PLAT_VENDOR_POLICY) to pass an intermediate
# check via "checkpolicy". In order to prevent Re-declaration,
# nonplat_declaration.cil is introduced to include the three
# files. It will be combined with vendor and odm sepolicy temporarily
# during "checkpolicy" check. nonplat_declaration.cil will be a separate
# output file, to be merged with vendor_sepolicy.cil and/or
# odm_sepolicy.cil when device boots.
#################################
include $(CLEAR_VARS)

LOCAL_MODULE := nonplat_declaration.cil
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_TAGS := optional
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE_PATH := $(TARGET_OUT_VENDOR)/etc/selinux
...
nonplat_declaration.conf := $(intermediates)/nonplat_declaration.conf
$(nonplat_declaration.conf): PRIVATE_MLS_SENS := $(MLS_SENS)
$(nonplat_declaration.conf): PRIVATE_MLS_CATS := $(MLS_CATS)
$(nonplat_declaration.conf): PRIVATE_TGT_ARCH := $(my_target_arch)
$(nonplat_declaration.conf): PRIVATE_TGT_WITH_ASAN := $(with_asan)
$(nonplat_declaration.conf): PRIVATE_ADDITIONAL_M4DEFS := $(LOCAL_ADDITIONAL_M4DEFS)
$(nonplat_declaration.conf): PRIVATE_FULL_TREBLE := $(PRODUCT_FULL_TREBLE)
$(nonplat_declaration.conf): $(call build_policy, $(sepolicy_build_files), \
$(PLAT_PUBLIC_POLICY) $(REQD_MASK_POLICY) $(PLAT_VENDOR_POLICY))
	$(transform-policy-to-conf)
	$(hide) sed '/dontaudit/d' $@ > $@.dontaudit

nonplat_declaration_raw := $(intermediates)/nonplat_declaration_raw.cil
$(nonplat_declaration_raw): PRIVATE_POL_CONF := $(nonplat_declaration.conf)
$(nonplat_declaration_raw): PRIVATE_REQD_MASK := $(reqd_policy_mask.cil)
$(nonplat_declaration_raw): $(HOST_OUT_EXECUTABLES)/checkpolicy $(nonplat_declaration.conf) \
...
$(LOCAL_BUILT_MODULE) : PRIVATE_TGT_POL := $(nonplat_declaration_raw)
$(LOCAL_BUILT_MODULE) : PRIVATE_DEP_CIL_FILES := $(built_plat_cil) $(built_mapping_cil)
$(LOCAL_BUILT_MODULE) : $(plat_pub_policy.cil) $(nonplat_declaration_raw) \
...
built_nonplat_declaration_cil := $(LOCAL_BUILT_MODULE)
nonplat_declaration.conf :=
nonplat_declaration_raw :=
...
$(vendor_policy.conf): PRIVATE_FULL_TREBLE := $(PRODUCT_FULL_TREBLE)
...
$(HOST_OUT_EXECUTABLES)/version_policy $(HOST_OUT_EXECUTABLES)/secilc \
$(built_plat_cil) $(built_mapping_cil) $(built_nonplat_declaration_cil)
	@mkdir -p $(dir $@)
	$(HOST_OUT_EXECUTABLES)/version_policy -b $< -t $(PRIVATE_TGT_POL) -n $(PRIVATE_VERS) -o $@.tmp
	$(hide) grep -Fxv -f $(built_nonplat_declaration_cil) $@.tmp > $@
...
$(odm_policy.conf): PRIVATE_FULL_TREBLE := $(PRODUCT_FULL_TREBLE)
$(odm_policy.conf): $(call build_policy, $(sepolicy_build_files), \
$(PLAT_PUBLIC_POLICY) $(REQD_MASK_POLICY) $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS))
	$(transform-policy-to-conf)
	$(hide) sed '/dontaudit/d' $@ > $@.dontaudit

odm_policy_raw := $(intermediates)/odm_policy_raw.cil
$(odm_policy_raw): PRIVATE_POL_CONF := $(odm_policy.conf)
$(odm_policy_raw): PRIVATE_REQD_MASK := $(reqd_policy_mask.cil)
$(odm_policy_raw): $(HOST_OUT_EXECUTABLES)/checkpolicy $(odm_policy.conf) \
$(reqd_policy_mask.cil)
	@mkdir -p $(dir $@)
	$(hide) $(CHECKPOLICY_ASAN_OPTIONS) $< -C -M -c $(POLICYVERS) -o $@.tmp $(PRIVATE_POL_CONF)
	$(hide) grep -Fxv -f $(PRIVATE_REQD_MASK) $@.tmp > $@
...
$(LOCAL_BUILT_MODULE) : PRIVATE_VERS := $(BOARD_SEPOLICY_VERS)
$(LOCAL_BUILT_MODULE) : PRIVATE_TGT_POL := $(odm_policy_raw)
$(LOCAL_BUILT_MODULE) : PRIVATE_DEP_CIL_FILES := $(built_plat_cil) $(built_mapping_cil)
$(LOCAL_BUILT_MODULE) : $(plat_pub_policy.cil) $(odm_policy_raw) \
$(HOST_OUT_EXECUTABLES)/version_policy $(HOST_OUT_EXECUTABLES)/secilc \
$(built_plat_cil) $(built_mapping_cil) $(built_nonplat_declaration_cil)
	@mkdir -p $(dir $@)
	$(HOST_OUT_EXECUTABLES)/version_policy -b $< -t $(PRIVATE_TGT_POL) -n $(PRIVATE_VERS) -o $@.tmp
...
file_contexts.vendor.tmp := $(intermediates)/file_contexts.vendor.tmp
$(file_contexts.vendor.tmp): PRIVATE_ADDITIONAL_M4DEFS := $(LOCAL_ADDITIONAL_M4DEFS)
$(file_contexts.vendor.tmp): $(vendor_fcfiles_with_nl)
	@mkdir -p $(dir $@)
	$(hide) m4 -s $(PRIVATE_ADDITIONAL_M4DEFS) $^ > $@

file_contexts.vendor.sorted.tmp := $(intermediates)/file_contexts.vendor.sorted.tmp
$(file_contexts.vendor.sorted.tmp): PRIVATE_SEPOLICY := $(built_sepolicy)
$(file_contexts.vendor.sorted.tmp): $(file_contexts.vendor.tmp) $(built_sepolicy) $(HOST_OUT_EXECUTABLES)/fc_sort $(HOST_OUT_EXECUTABLES)/checkfc
	@mkdir -p $(dir $@)
...
file_contexts.odm.tmp := $(intermediates)/file_contexts.odm.tmp
$(file_contexts.odm.tmp): PRIVATE_ADDITIONAL_M4DEFS := $(LOCAL_ADDITIONAL_M4DEFS)
$(file_contexts.odm.tmp): $(odm_fcfiles_with_nl)
	@mkdir -p $(dir $@)
	$(hide) m4 -s $(PRIVATE_ADDITIONAL_M4DEFS) $^ > $@

file_contexts.odm.sorted.tmp := $(intermediates)/file_contexts.odm.sorted.tmp
$(file_contexts.odm.sorted.tmp): PRIVATE_SEPOLICY := $(built_sepolicy)
$(file_contexts.odm.sorted.tmp): $(file_contexts.odm.tmp) $(built_sepolicy) $(HOST_OUT_EXECUTABLES)/fc_sort $(HOST_OUT_EXECUTABLES)/checkfc
	@mkdir -p $(dir $@)
	$(hide) $(HOST_OUT_EXECUTABLES)/checkfc -e $(PRIVATE_SEPOLICY) $<
	$(hide) $(HOST_OUT_EXECUTABLES)/fc_sort $< $@
endif
...
all_file_contexts := \
    ...
    $(file_contexts.vendor.sorted.tmp)

ifdef BOARD_ODM_SEPOLICY_DIRS
all_file_contexts += $(file_contexts.odm.sorted.tmp)
endif

file_contexts.concat.tmp := $(intermediates)/file_contexts.concat.tmp
$(file_contexts.concat.tmp): $(all_file_contexts)
	@mkdir -p $(dir $@)
	$(hide) m4 -s $^ > $@
...
file_contexts.concat.tmp :=
file_contexts.vendor.sorted.tmp :=
file_contexts.vendor.tmp :=
file_contexts.odm.sorted.tmp :=
file_contexts.odm.tmp :=
file_contexts.local.tmp :=
all_file_contexts := 
...
odm_sc_files := $(call build_policy, seapp_contexts, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))
plat_sc_neverallow_files := $(call build_policy, seapp_contexts, $(PLAT_PRIVATE_POLICY))
...
$(LOCAL_BUILT_MODULE): PRIVATE_SEPOLICY := $(built_sepolicy)
$(LOCAL_BUILT_MODULE): $(vendor_property_contexts.tmp) $(built_sepolicy) $(HOST_OUT_EXECUTABLES)/checkfc
	@mkdir -p $(dir $@)
	$(hide) sed -e 's/#.*$$//' -e '/^$$/d' $< | sort -u -o $@
	$(hide) $(HOST_OUT_EXECUTABLES)/checkfc -p $(PRIVATE_SEPOLICY) $@
...
odm_pcfiles := $(call build_policy, property_contexts, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))
...
$(LOCAL_BUILT_MODULE): PRIVATE_SEPOLICY := $(built_sepolicy)
$(LOCAL_BUILT_MODULE): $(odm_property_contexts.tmp) $(built_sepolicy) $(HOST_OUT_EXECUTABLES)/checkfc
	@mkdir -p $(dir $@)
	$(hide) sed -e 's/#.*$$//' -e '/^$$/d' $< | sort -u -o $@
	$(hide) $(HOST_OUT_EXECUTABLES)/checkfc -p $(PRIVATE_SEPOLICY) $@
...
LOCAL_MODULE := odm_service_contexts
LOCAL_MODULE_CLASS := ETC
LOCAL_MODULE_TAGS := optional
ifeq ($(PRODUCT_FULL_TREBLE),true)
LOCAL_MODULE_PATH := $(TARGET_OUT_ODM)/etc/selinux
...
odm_svcfiles := $(call build_policy, service_contexts, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))

odm_service_contexts.tmp := $(intermediates)/odm_service_contexts.tmp
$(odm_service_contexts.tmp): PRIVATE_SVC_FILES := $(odm_svcfiles)
$(odm_service_contexts.tmp): PRIVATE_ADDITIONAL_M4DEFS := $(LOCAL_ADDITIONAL_M4DEFS)
$(odm_service_contexts.tmp): $(odm_svcfiles)
	@mkdir -p $(dir $@)
	$(hide) m4 -s $(PRIVATE_ADDITIONAL_M4DEFS) $(PRIVATE_SVC_FILES) > $@

$(LOCAL_BUILT_MODULE): PRIVATE_SEPOLICY := $(built_sepolicy)
$(LOCAL_BUILT_MODULE): $(odm_service_contexts.tmp) $(built_sepolicy) $(HOST_OUT_EXECUTABLES)/checkfc $(ACP)
	@mkdir -p $(dir $@)
	sed -e 's/#.*$$//' -e '/^$$/d' $< > $@
	$(hide) $(HOST_OUT_EXECUTABLES)/checkfc -s $(PRIVATE_SEPOLICY) $@

built_odm_svc := $(LOCAL_BUILT_MODULE)
odm_svcfiles :=
odm_service_contexts.tmp :=
endif
...
odm_hwsvcfiles := $(call build_policy, hwservice_contexts, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))
...
$(odm_mac_perms_keys.tmp): $(call build_policy, keys.conf, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))
	@mkdir -p $(dir $@)
	$(hide) m4 -s $(PRIVATE_ADDITIONAL_M4DEFS) $^ > $@

all_odm_mac_perms_files := $(call build_policy, mac_permissions.xml, $(PLAT_VENDOR_POLICY) $(BOARD_ODM_SEPOLICY_DIRS) $(REQD_MASK_POLICY))
...
$(treble_sepolicy_tests): PRIVATE_VENDOR_FC := $(built_vendor_fc)
$(treble_sepolicy_tests): PRIVATE_ODM_FC := $(built_odm_fc)
$(treble_sepolicy_tests): PRIVATE_SEPOLICY := $(built_sepolicy)
ifdef BOARD_ODM_SEPOLICY_DIRS
$(treble_sepolicy_tests): $(HOST_OUT_EXECUTABLES)/treble_sepolicy_tests.py \
$(built_plat_fc) $(built_vendor_fc) $(built_odm_fc) $(built_sepolicy)
	@mkdir -p $(dir $@)
	$(hide) python $(HOST_OUT_EXECUTABLES)/treble_sepolicy_tests.py -l $(HOST_OUT)/lib64 -f $(PRIVATE_PLAT_FC) -f $(PRIVATE_VENDOR_FC) -f $(PRIVATE_ODM_FC) -p $(PRIVATE_SEPOLICY)
	$(hide) touch $@
else
$(treble_sepolicy_tests): $(HOST_OUT_EXECUTABLES)/treble_sepolicy_tests.py \
$(built_plat_fc) $(built_vendor_fc) $(built_sepolicy)
	@mkdir -p $(dir $@)
	$(hide) python $(HOST_OUT_EXECUTABLES)/treble_sepolicy_tests.py -l $(HOST_OUT)/lib64 -f $(PRIVATE_PLAT_FC) -f $(PRIVATE_VENDOR_FC) -p $(PRIVATE_SEPOLICY)
	$(hide) touch $@
endif
